...
function circuitParams = setUpParams(variablesToSave)
    ...
    global BRes CRes;        % BRes being the Base resistor and CRes being the collector resistor
    
    % confirms that the global variabels have been initialized
    globalCheck = find([isempty(DAC0), isempty(DAC1), isempty(BRes), isempty(CRes)],1);
    if globalCheck
        varsForError = ["DAC0" "DAC1" "BRes" "CRes"];
    ...
    mustBeMember(CRes,[0 1 2]);
    mustBeMember(BRes,[0 1]);
%     
...
% 
    ...
    circuitParams = ...
        [struct( 'lineInCirfile', {".PARAM DAC0Param=",".PARAM DAC1Param=",".PARAM CRes=",".PARAM BRes="}, ...
                'value', {DACbinToVolt(DAC0), DACbinToVolt(DAC1), CRes, BRes}), ...
